...
function createProductPopUp(product, options){
  ...
  for(let i=0;i<product.options.length;i++){
    ...
    name.className="text-halfgray text-sm xl:text-base";
    ...
    if(product.options[i].name==="Color"){
      optionList.className="flex gap-3 xl:gap-4 mt-4 overflow-x-auto w-full";
      ...
      optionList.className="flex gap-2 xl:gap-3 flex-wrap mt-4 text-sm xl:text-base xl:leading-5 overflow-x-auto w-full"
      for(let j=0; j<product.options[i].values.length;j++){
        option=document.createElement('span');
        let selected = j===options[i]?"bg-black text-white":"bg-gray";
        option.className="py-2.5 xl:py-3 px-5 xl:px-6 rounded-full cursor-pointer flex-shrink-0 "+selected;
...
function createProductCard(product, ...classList){
  ...
  article.classList.add("w-44", "xl:w-72", "cursor-pointer", ...classList);
  ...
  div.classList.add("text-xl", "xl:text-2xl", "font-bold", "flex", "gap-1", "xl:gap-2.5", "items-center", "flex-wrap");
  ...
  if(oldPrice !== null && parseFloat(oldPrice)>parseFloat(newPrice)){
    ...
    discount.className="bg-lightred text-textred px-3.5 py-1.5 rounded-full xl:leading-5 font-medium text-xxs xl:text-xs";
...
function renderPage(index=1){
  ...
  if(index == count || index + 1 == count){
    for (let i = 1; i <= 2+Math.floor(limit/9); i++) {
      fragmet.append(createTableLi(() => getProducts(i), { cl: "rounded-lg flex items-center justify-center xl:w-10 xl:h-10 w-9 h-9 cursor-pointer", txt: i }));
    }
    ...
    fragmet.append(createTableLi(() => getProducts(1), { cl: "rounded-lg flex items-center justify-center xl:w-10 xl:h-10 w-9 h-9 cursor-pointer", txt: "1" }));
    points = document.createElement('li');
    points.textContent = "...";
    points.className="rounded-lg flex items-center justify-center xl:w-10 xl:h-10 w-9 h-9";
    fragmet.append(points);
    ...
    points.className="rounded-lg flex items-center justify-center xl:w-10 xl:h-10 w-9 h-9";
    fragmet.append(points);
    fragmet.append(createTableLi(() => getProducts(count), { cl: "rounded-lg flex items-center justify-center xl:w-10 xl:h-10 w-9 h-9 cursor-pointer", txt: count }));
  }
  else {
      for (let i = 1; i <= 2+Math.floor(limit/9); i++) {
        let selected = i === index ? "text-black bg-lightblack" : "cursor-pointer";
